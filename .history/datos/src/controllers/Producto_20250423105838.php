...
class Producto
{
    ...
    public function create(Request $request, Response $response, $args)
    {
        ...
        foreach ($body as $key => $value) {
            $TIPO = gettype($value) == 'integer' ? PDO::PARAM_INT : PDO::PARAM_STR; //Determinar el tipo de dato
            $query->bindParam($key, $value, $TIPO); // Asignar el valor a la consulta
        }
        ...
        foreach ($body as $key => $value) { 
            $query->bindParam($key, $value); // Asignar el valor a la consulta
        }
